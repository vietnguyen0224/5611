...
TODO:
  ...
  3. Compute an avoidance force as follows:
      ...
      very large value of k_goal (should be k_avoid???) to avoid collisions between agents. //
...
Vec2[] goalForce = new Vec2[maxNumAgents];
...
void resetSimulation() {
  ...
  goalPos[0] = new Vec2(random(35, 750), random(35, 250));
  for (int i = 1; i < numAgents; i++) {
    goalPos[i] = new Vec2(goalPos[0].x, goalPos[0].y + 3*agentRad*i);
  }
...
Vec2 computeAgentForces(int id){
  ...
  // if agents near their goals, stop reacting with other agents
  if (goalVel.length() < 50) return acc;
  
  for (int j = 0; j < numAgents; j++) {
    if (j == id) continue;
    
    // if agents start ovelapping, fix their positions
    if (agentPos[id].distanceTo(agentPos[j]) < 2*agentRad) {
      Vec2 normal = (agentPos[id].minus(agentPos[j])).normalized();
      agentPos[id] = agentPos[j].plus(normal.times(2*agentRad).times(1.01));
    }
    ...
    if (ttc > 0) acc.add(avoidForce.times(k_avoid));
