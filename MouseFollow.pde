...
float w;
float h;
float max = 200; //Max acceleration
...
void update(float dt){
  ...
  // speed*dt is mouse move's length in dt (time between 2 frames)
...
void draw(){
  ...
  stroke(0,0,0);
  fill(100,20,10);
  ellipse(pos.x, pos.y,w,h); //Question: Why is it radius*2 ?
}
